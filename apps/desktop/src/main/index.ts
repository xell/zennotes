...
// Direct keyboard interception rather than relying solely on the menu
// accelerators: Windows/Linux keep autoHideMenuBar (no native menu at all,
// see installAppMenu), so without this, zoom shortcuts would only work on
// macOS.
...
async function ensureQuickCaptureWindow(): Promise<BrowserWindow> {
  ...
  //
  // NOTE (merge 2026-08-15): upstream's #497-adjacent quick-capture work adds
  // a hide-on-blur handler here instead (Spotlight/Raycast-style, skipped
  // when pinned) — dropped as a real behavior conflict with the comment
  // above, not auto-resolved. Flagged for a decision, not changed here.
